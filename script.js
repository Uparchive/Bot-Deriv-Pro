document.addEventListener('DOMContentLoaded', () => {
  // Toggle das seções por data
  document.querySelectorAll('.video-section-header').forEach(header => {
    header.addEventListener('click', () => {
      const section = header.closest('.video-section');
  ...
  function filterVideos() {
    ...
    timer = setTimeout(() => {
      ...
      document.querySelectorAll('.video-section').forEach(sec => {
        const seçãoDate  = sec.dataset.date;
        const títuloSec  = sec.querySelector('h2').textContent.toLowerCase();
        const dateMatch  = !dt || seçãoDate === dt;
        const textMatch  = !txt ||
          títuloSec.includes(txt) ||
